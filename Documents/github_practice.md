...
今回は特にどちらが正解というのはないので、先方の変更、つまり**their changes**の方を採用してみましょう。

3. 青い部分の左の**Use me**をクリックして、devBの方を採用します

![devBの変更を採用する](./images/img21.png)

4. Ctrl + F キーで検索ダイアログを出して`=====`を検索します。他に見つからなければこのファイルのコンフリクトは解決なので、Ctrl + S キーで保存します
1. GitHub Desktopに切り替えます
...
コンフリクトの解決は手間がかかりますし、Unityでは解消が難しくなることもあるので、決まったメンバーで開発するのであればルールを決めて、コンフリクトを避けるのが得策です。以下にルールを挙げます。

- mainブランチはマージ専用にして、作業はブランチを作成してそこで行う
- マージの担当者を決めて、他のメンバーはマージをしないようにする
- シーンやプレハブの配置やInspectorを操作する作業は、担当者を決めて、その担当者のみが行うようにする
- 他のメンバーが担当しているシーンやプレハブ、ScriptableObject上で開発する必要がある場合は、直に作業をせず、それらを複製して作業をする。渡す時も複製したものを渡して、担当者にマージしてもらうか、変更を反映してもらう
